...
class CBCore extends UIObject {
    ...
    set deviceNames(deviceNames: { [key: string]: string }) {
        
        if (IS_NOT(deviceNames)) {
            
            localStorage.removeItem("CBDeviceNames")
            
        }
        
        localStorage.setItem("CBDeviceNames", JSON.stringify(deviceNames))
        
    }
    
    get deviceNames() {
        
        const result = JSON.parse(FIRST(localStorage.getItem("CBDeviceNames"), "{ }"))
