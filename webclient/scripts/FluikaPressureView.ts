...
class FluikaPressureView extends UIView {
    ...
    private _deviceObject: SerialPortFluikaDeviceConnectionObject
    ...
    constructor(elementID?: string) {
        ...
        this.pressureTargetView.textField.addControlEventTarget.TextChange.Blur.EnterDown = async (sender, event) => {
            ...
            if (IS(this.deviceObject) && !isTextFieldFocused || IS_NOT(event)) {
                ...
                await SocketClient.SetPressureOnFluikaDevice({
    ...
    get descriptorObject() {
        ...
        let objectValue = {
            ...
            viewType: FluikaPressureView.name,
    ...
    set descriptorObject(descriptorObject: FluikaDescriptorObject) {
    ...
    set deviceObject(deviceObject: SerialPortFluikaDeviceConnectionObject) {
    ...
    private async updatePressure() {
        ...
        let pressureObject = (await SocketClient.RetrievePressureOnFluikaDevice({
            devicePath: this.deviceObject.port.path
        })).result
        
        this.deviceObject.pressureInMillibars = pressureObject.pressure
        
        this.pressureLabel.textPrefix = pressureObject.errorMessage + " "
        
        if (("" + pressureObject.errorMessage).contains("4")) {
            
            // Stop pumps
            
            
            let plotterViewController = FluikaViewController.instance
            
            plotterViewController.stopPumps(this._descriptorObject.inputString)
            
        }
        
        
        this.pressureLabel.text = "" + (pressureObject.pressure + this.deviceObject.minPressureInMillibars).toPrecision(
            5)
