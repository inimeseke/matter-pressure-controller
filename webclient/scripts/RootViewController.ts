...
class RootViewController extends UIViewController {
    ...
    private fluikaViewController: FluikaViewController
    private pressureViewController: PressureViewController
    ...
    loadSubviews() {
        ...
        this.topBarView = new CellView("TopBarView").copyValuesRecursivelyFromObject({
            titleLabel: {
                text: "Vesicle results",
                textAlignment: UITextView.textAlignment.center
            },
            colors: {
                background: { normal: new UIColor("#0a00f1") },
    ...
    async handleRoute(route: UIRoute) {
        ...
        if (IS(route.componentWithName(FluikaViewController.routeComponentName))) {
            
            // Show view
            if (!IS(this.fluikaViewController)) {
                
                this.fluikaViewController = new FluikaViewController(new UIView("FluikaView"))
                
            }
            
            this.contentViewController = this.fluikaViewController
            
        }
        else if (IS(route.componentWithName(PressureViewController.routeComponentName))) {
            
            // Show view
            if (!IS(this.pressureViewController)) {
                
                this.pressureViewController = new PressureViewController(new UIView("PressureView"))
                
            }
            
            this.contentViewController = this.pressureViewController
            ...
            if (!IS(this.mainViewController)) {
                ...
                this.mainViewController.view.viewHTMLElement.innerHTML = await CBCore.sharedInstance.readTextFileFromURL(
                    "/frontpage.html")
